 ...
 |-- div5tailnum: integer (nullable = true)
   */
    ...
    public static JavaRDD<Row> QA(SparkSession spark, String dataFile) {
        Dataset<Row> d = spark.read().parquet(dataFile);
	d.createOrReplaceTempView("flights");
	Dataset<Row> r = d.where("origincityname = 'Seattle, WA'")
			  .select("destcityname")
			  .distinct();
	// r.show();
	JavaRDD<Row> rdd = r.javaRDD();
        return rdd;
    }
    ...
    public static JavaRDD<Row> QB(SparkSession spark, String dataFile) {
        Dataset<Row> d = spark.read().parquet(dataFile);
	d.createOrReplaceTempView("flights");	
	Dataset<Row> r = d.filter("cancelled != 1")
			  .groupBy("origincityname", "month")
			  .count();
	// Dataset<Row> r = spark.sql("SELECT origincityname, month, COUNT (*) AS count FROM flights WHERE cancelled <> 1 GROUP BY origincityname, month");
	// r.show();
	JavaRDD<Row> rdd = r.javaRDD();
        return rdd;
    }
    ...
    public static JavaRDD<Row> QC(SparkSession spark, String dataFile) {
        Dataset<Row> d = spark.read().parquet(dataFile);
	d.createOrReplaceTempView("flights");
	Dataset<Row> r = d.filter("depdelayminutes IS NOT null")
			  .groupBy("origincityname")
			  .avg("depdelayminutes");
	// Dataset<Row> r = spark.sql("SELECT origincityname, AVG(depdelayminutes) as avg FROM Flights WHERE depdelayminutes IS NOT null GROUP BY origincityname");
    	// r.show();
	JavaRDD<Row> rdd = r.javaRDD();
        return rdd;
    }
